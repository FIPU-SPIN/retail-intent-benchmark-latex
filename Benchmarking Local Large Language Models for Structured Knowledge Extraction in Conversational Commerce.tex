...
\begin{figure}[H]
\centering
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/quality_json_validity_by_regime.pdf}}\hfill
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/quality_schema_validity_by_regime.pdf}}\\
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/quality_operation_f1_by_regime.pdf}}\hfill
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/quality_exact_match_by_regime.pdf}}\\
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/quality_action_acc_by_regime.pdf}}\hfill
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/quality_product_acc_by_regime.pdf}}\\
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/quality_quantity_acc_by_regime.pdf}}
\caption{All quality metrics by model and prompting regime; each panel is a grouped bar chart over the six models (bars: minimal, extended, few-shot): (\textbf{a})~JSON validity, (\textbf{b})~schema validity, (\textbf{c})~operation F1, (\textbf{d})~exact match, (\textbf{e})~action accuracy, (\textbf{f})~product accuracy, and (\textbf{g})~quantity accuracy. Few-shot prompting (teal) is best across essentially every metric for the capable models, and the largest small-model gains appear on schema validity and exact match, confirming that the principal failure mode under weaker prompts is structural rather than semantic.\label{fig:qualgrid}}
\end{figure}
...
\begin{figure}[H]
\centering
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/prompt_tokens_by_regime.pdf}}\hfill
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/output_tokens_by_regime.pdf}}
\caption{Mean prompt (input; \textbf{a}) and output (decoded; \textbf{b}) token counts per command, by model and prompting regime. Few-shot prompts are markedly longer on the input side (\textbf{a}) yet \emph{shorter} on the output side (\textbf{b}): the demonstrations elicit concise, minified JSON and suppress verbose generation. Because decoding dominates latency, this reduction in output tokens is the mechanism behind the lower few-shot latency and energy reported in Table~\ref{tab:cost}.\label{fig:tokens}}
\end{figure}
...
\begin{figure}[H]
\centering
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/cost_mean_latency__s_by_regime.pdf}}\hfill
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/cost_p95_latency__s_by_regime.pdf}}\\
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/cost_throughput__cmd_s_by_regime.pdf}}\hfill
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/cost_gpu_power__w_by_regime.pdf}}\\
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/cost_energy_command__j_by_regime.pdf}}\hfill
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/cost_vram__gb_by_regime.pdf}}\\
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/cost_prompt_tokens_by_regime.pdf}}\hfill
\subfloat[]{\includegraphics[width=0.49\linewidth]{figures/cost_output_tokens_by_regime.pdf}}
\caption{All cost metrics by model and prompting regime: (\textbf{a})~mean latency, (\textbf{b})~p95 latency, (\textbf{c})~throughput, (\textbf{d})~mean GPU power, (\textbf{e})~estimated energy per command, (\textbf{f})~VRAM occupancy, (\textbf{g})~mean prompt tokens, and (\textbf{h})~mean output tokens. Latency, energy, and throughput improve under few-shot prompting for every model, whereas GPU power is essentially model-bound. The latency and energy panels are dominated by Gemma4 31B, reflecting the ${\sim}40\times$ cost dynamic range across the frontier. VRAM reflects serving-runtime reservation rather than the minimal model footprint (see text).\label{fig:costgrid}}
\end{figure}
